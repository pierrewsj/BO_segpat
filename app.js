...
function assistantContext(record = state.current) {
  ...
  const reference = basic.referencia === 'Danos materiais — Outra'
    ? basic.referenciaOutra
    : basic.referencia;
  ...
  const vehicle = accident || /(veicul|reboque|entrada com danos|faixa horaria)/.test(searchable) || Boolean(record?.vehicles?.length);
  ...
  const damage = /(dano|avaria|erro operacional|ato doloso)/.test(searchable);

  let profile = 'Ocorrência geral';
  if (fire) profile = 'Emergência ou incêndio';
  ...
  const vehicleRequired = accident || /(veiculos|transporte por reboque|entrada com danos)/.test(searchable);
  const materialRequired = theft || /(carga de materiais|avaria em pecas|vasilhames|recolhimento de material|desaparecimento de material)/.test(searchable);
