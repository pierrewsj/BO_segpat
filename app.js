...
// v38.1 — durante a fase de validação operacional, fotos ficam recomendadas, não obrigatórias.
// Documentos exigidos pelo roteiro continuam com a mesma obrigatoriedade.
const TEST_MODE_PHOTOS_OPTIONAL = true;

const DRIVER_REQUIRED_REFERENCES = new Set([
  'Acidente ou colisão envolvendo veículo',
  'Avaria constatada em veículo',
  'Veículo apresentado com dano preexistente',
  'Remoção ou transporte de veículo por reboque',
...
const DRIVER_REQUIRED_SUBMODELS = new Set([
  'pane-caminhao','sider-aberto','reboque-customer-care',
  'destino-incorreto','conferencia-geral','recusa-carregamento',
  'fiscalizacao-transito','fiscalizacao-radar','estado-conservacao-veiculo',
  'overtime','mvm-extraviado-guiche','uso-indevido-veiculo','motorista-sem-epi'
]);
...
function effectiveTemplate(record = state.current) {
  ...
  return {
    ...
    evidence:TEST_MODE_PHOTOS_OPTIONAL ? 'recommended' : (submodel?.evidence || base.evidence || 'recommended'),
...
function driverDataRequired(record = state.current) {
  const reference = normalizeReference(record?.basic?.referencia || '');
  const submodel = selectedSubmodel(record);
  if (DRIVER_REQUIRED_REFERENCES.has(reference)) return true;
  if (submodel?.id && DRIVER_REQUIRED_SUBMODELS.has(submodel.id)) return true;
  // Nos roteiros genéricos de veículo, só passa a ser obrigatório quando o próprio preenchimento confirma um condutor.
  if (templateAnswer(record,'condutorIdentificado') === 'Sim') return true;
  return false;
}

function isDriver(person = {}) {
  return normalizeAssistantText(person?.tipo || '') === 'condutor';
}

function driverDataComplete(person = {}) {
  if (!isDriver(person)) return false;
  return Boolean(
    String(person.nome || '').trim() &&
    String(person.cpf || '').replace(/\D/g,'').length === 11 &&
    String(person.numeroCNH || person.numeroDocumento || '').trim() &&
    String(person.nomeMae || '').trim() &&
    String(person.nomePai || '').trim()
  );
}
...
function effectiveEntityModes(record = state.current) {
  ...
  if (driverDataRequired(record)) modes.people = 'required';
  return modes;
}
...
function evidenceRequirementsFor(record = state.current) {
  const reference = normalizeReference(record?.basic?.referencia || '');
  const sub = selectedSubmodel(record);
  let base = sub?.replaceEvidenceRequirements ? [] : (EVIDENCE_REQUIREMENTS[reference] || []).map(item => ({...item}));
  if (sub?.evidence === 'recommended') base = base.map(item => ({...item,required:false}));
  ...
  if (!TEST_MODE_PHOTOS_OPTIONAL) return values;
  return values.map(item => item.type === 'photo' ? {...item, required:false, testingOptional:true} : item);
}
...
function assistantChecklist(record = state.current) {
  ...
  if (modes.people === 'required') add('','Pessoa diretamente envolvida',involved,'required',1,'Cadastre a pessoa diretamente envolvida.','people');
  if (driverDataRequired(r)) {
    const driver = (r.people||[]).find(isDriver);
    add('','Dados do motorista / CNH',Boolean(driver && driverDataComplete(driver)),'required',1,'Cadastre o motorista como Condutor e informe nome, CPF, número da CNH, nome da mãe e nome do pai.','people');
  }
  ...
  add('history-end','Providências e desfecho',meaningfulText(h.desfecho),'required',3,'Informe as providências adotadas e como a situação terminou.');
...
function assistantPersonSummary(person) {
  ...
  if (isDriver(person)) {
    if (person.cpf) details.push(`CPF ${person.cpf}`);
    if (person.numeroCNH || person.numeroDocumento) details.push(`CNH ${person.numeroCNH || person.numeroDocumento}`);
    if (person.categoriaCNH) details.push(`categoria CNH ${person.categoriaCNH}`);
    if (person.validadeCNH) details.push(`validade da CNH ${formatDateOnly(person.validadeCNH)}`);
    if (person.nomeMae || person.nomePai) details.push(`filiação: mãe ${person.nomeMae || 'não informada'}; pai ${person.nomePai || 'não informado'}`);
  } else if (person.numeroDocumento) details.push(`${person.tipoDocumento || 'documento'} ${person.numeroDocumento}`);
...
const REPORT_SECTION_HEADINGS = new Set(['CONTEXTO','RELATO RECEBIDO','CONSTATAÇÃO DA SEGURANÇA','APURAÇÃO COMPLEMENTAR','ELEMENTOS RELACIONADOS','PROVIDÊNCIAS','DESFECHO','PROVIDÊNCIAS E DESFECHO','INFORMAÇÃO COMPLEMENTAR']);
...
function buildNarrative(record = state.current) {
  ...
  // Mantém providências estruturadas/áreas acionadas, mas o vigilante responde uma única pergunta operacional.
  const providences = meaningfulText(h.providencias) || buildAssistantProvidences(record);
  if (providences && !similarReportText(providences,h.desfecho)) providenceParts.push(providences);
  const outcome = cleanSentence(h.desfecho);
  if (outcome) {
    const cleanedOutcome = outcome.replace(/^ao\s+t[eé]rmino\s+do\s+atendimento\s*[,;:\-–—]*\s*/i, '').trim();
    if (cleanedOutcome && !providenceParts.some(part=>similarReportText(part,cleanedOutcome))) providenceParts.push(cleanedOutcome);
  }
  ...
  if (providenceParts.length) addSection('PROVIDÊNCIAS E DESFECHO', providenceParts.join(' '));
...
function renderRelatedStep() {
  ...
  const driverRequired = driverDataRequired(state.current);
  const blocks = [];
  if (modes.people === 'required') blocks.push(requiredEntitySection('people',{key:'people',title:driverRequired?'Motorista/condutor e pessoas diretamente envolvidas':'Pessoas diretamente envolvidas',description:driverRequired?'Cadastre o motorista como Condutor. Para o condutor, nome, CPF, número da CNH e filiação (mãe e pai) são obrigatórios e serão incluídos no relato. Não fotografe a CNH.':'Cadastre ao menos uma pessoa envolvida. Se o solicitante for o próprio envolvido, você pode indicar isso abaixo sem duplicar o cadastro.',button:driverRequired?'Adicionar motorista/condutor':'Adicionar pessoa',empty:driverRequired?'Nenhum motorista/condutor cadastrado.':'Nenhuma pessoa envolvida cadastrada.',render:renderPersonItem,filter:person=>person.tipo!=='Testemunha'}));
...
function renderEvidenceBlock() {
  ...
  const requirementCards = requirements.length ? `<div class="evidence-requirement-list">${requirements.map(item=>`<article class="evidence-requirement ${item.satisfied?'done':item.required?'pending':''} ${dynamicRequirementActive(`evidence:${item.id}`)?'dynamic-required':''}"><span class="evidence-requirement-icon">${item.satisfied?ICONS.check:ICONS.file}</span><div><strong>${escapeHtml(item.label)}</strong><small>${item.required?'Essencial':item.testingOptional?'Recomendado • modo de teste':'Recomendado'}${item.safety?' • somente se houver condição segura':''}</small></div>${item.satisfied?'<span class="evidence-ok">Registrada</span>':`<button class="button ${item.required?'primary':'secondary'} small" type="button" data-evidence-purpose="${escapeHtml(item.id)}">${ICONS.plus} ${item.type==='photo'?'Tirar foto':'Adicionar'}</button>`}</article>`).join('')}</div>` : '';
  ...
  return `<section class="evidence-inline-card"><div class="entity-toolbar"><div><p class="eyebrow">Evidências</p><h2>Fotos e documentos</h2><p>${TEST_MODE_PHOTOS_OPTIONAL?'Durante o teste, as fotos são recomendadas, mas não impedem a continuidade do BO.':'Inclua evidências quando contribuírem para comprovação ou compreensão do fato.'}</p></div><span class="question-level ${evidenceRequired?'required':'recommended'}">${evidenceRequired?'Orientado':'Recomendado'}</span></div>${requirementCards}${reasonField}<div class="attachment-actions"><button class="button primary" type="button" data-pick-file="camera">${ICONS.plus} Foto adicional</button><button class="button secondary" type="button" data-pick-file="gallery">Galeria</button><button class="button secondary" type="button" data-pick-file="document">Documento adicional</button></div><input id="attachment-camera" class="hidden" type="file" accept="image/*" capture="environment"><input id="attachment-gallery" class="hidden" type="file" multiple accept="image/*"><input id="attachment-document" class="hidden" type="file" multiple accept=".pdf,.doc,.docx,.xls,.xlsx,text/plain"><div class="storage-meter"><span>${state.current.attachments.length} arquivo(s)</span><strong>${humanSize(size)} utilizados</strong></div><div class="file-grid">${state.current.attachments.map((file,index)=>renderFileItem(file,index)).join('')}</div>${missingRequired.length&&!unavailableReason?`<div class="notice warning compact">Faltam ${missingRequired.length} evidência(s) essencial(is). Registre-as ou justifique a impossibilidade.</div>`:''}</section>`;
}

function renderActionsEvidenceStep() {
  const r = state.current;
  const h = r.history;
  return `<section class="form-card step-intro-card"><p class="eyebrow">4. Providências e evidências</p><h1>Registre a atuação e o desfecho</h1><p>Em uma única resposta, informe o que foi feito e como a situação terminou. Depois, registre a área acionada, quando houver.</p></section>
  <section class="form-card"><div class="form-grid"><div class="field full"><label class="required" for="history-end">Quais providências foram adotadas e como terminou a situação?</label><textarea id="history-end" data-path="history.desfecho" placeholder="Ex.: material recolhido e encaminhado à área responsável; local liberado sem novos desdobramentos." required>${escapeHtml(h.desfecho||h.providenciasFonte||'')}</textarea><small>Registre na mesma resposta a ação realizada e a condição final da ocorrência.</small><span class="field-error" data-error-for="history-end"></span></div><div class="field full"><label for="history-called">Quem ou qual área foi acionada?</label><textarea id="history-called" data-path="history.acionados" placeholder="Ex.: liderança, ambulatório, bombeiros, manutenção, RH...">${escapeHtml(h.acionados)}</textarea></div><div class="field full"><label for="history-extra">Informação complementar</label><textarea id="history-extra" data-path="history.adicional" placeholder="Use somente para informação relevante que ainda não tenha sido registrada.">${escapeHtml(h.adicional)}</textarea></div></div></section>
  ${renderEvidenceBlock()}
...
function renderPersonItem(person, index) {
  return `<article class="entity-item">
    <div><h3>${escapeHtml(person.nome || 'Pessoa sem nome')}</h3><div class="entity-meta"><span class="chip">${escapeHtml(person.tipo)}</span><span class="chip">${escapeHtml(person.vinculo)}</span>${person.matricula ? `<span class="chip">Matrícula ${escapeHtml(person.matricula)}</span>` : ''}${isDriver(person)&&person.numeroCNH?`<span class="chip">CNH ${escapeHtml(person.numeroCNH)}</span>`:''}${isDriver(person)&&person.cpf?`<span class="chip">CPF ${escapeHtml(person.cpf)}</span>`:''}</div></div>
...
function renderMaterialItem(material, index) {
  ...
  return `<article class="entity-item">
    <div><h3>${escapeHtml(material.denominacao || 'Item sem denominação')}</h3><div class="entity-meta"><span class="chip">${escapeHtml(materialItemTypeLabel(material))}</span>${quantity}${technical}</div></div>
...
function renderHistoryStep() {
  ...
  <section class="form-card">
    ...
    <div class="form-grid">
      ...
      <div class="field full"><label class="required" for="history-end">Quais providências foram adotadas e como terminou a situação?</label><textarea id="history-end" data-path="history.desfecho" placeholder="Informe a ação realizada e a condição final da ocorrência." required>${escapeHtml(h.desfecho||h.providenciasFonte||'')}</textarea><span class="field-error" data-error-for="history-end"></span></div>
...
function reviewItems(record = state.current) {
  ...
  return [
    ...
    make('actions','Providências e desfecho',3,step3.filter(i=>i.key!=='attachments'),'history-end','Providências, acionamentos e condição final conferidos.'),
  ...
  return `<div class="review-professional-layout v32-review">
    ...
    <div class="review-detail-panel">
      ...
      <div class="review-grid compact-review-grid">
        ...
        ${reviewSection('Providências e desfecho',3,`<dl class="definition-grid"><div style="grid-column:1/-1"><dt>Providências adotadas e resultado</dt><dd>${escapeHtml(r.history.desfecho||r.history.providenciasFonte||r.history.providencias||'Não informado')}</dd></div><div><dt>Áreas acionadas</dt><dd>${escapeHtml(r.history.acionados||'Não informado')}</dd></div></dl>`)}
...
function renderProvidencesCorrection() {
  const h = state.current.history;
  return `<section class="form-card correction-card" data-correction-section="providencias"><p class="eyebrow">Correção rápida</p><h1>Providências e desfecho</h1><p>Use uma única resposta para registrar a ação realizada e a condição final da ocorrência.</p><div class="form-grid">
    <div class="field full"><label class="required" for="history-end">Quais providências foram adotadas e como terminou a situação?</label><textarea id="history-end" data-path="history.desfecho" required>${escapeHtml(h.desfecho||h.providenciasFonte||'')}</textarea><span class="field-error" data-error-for="history-end"></span></div>
    <div class="field full"><label for="history-called">Quem ou qual área foi acionada?</label><textarea id="history-called" data-path="history.acionados" placeholder="Ex.: ambulatório, bombeiros, liderança, manutenção...">${escapeHtml(h.acionados)}</textarea></div>
  </div></section>`;
}
...
function renderFocusedCorrection(focus) {
  const requiredPeople = () => requiredEntitySection('people',{key:'people',title:driverDataRequired(state.current)?'Motorista/condutor e pessoas diretamente envolvidas':'Pessoa diretamente envolvida',description:driverDataRequired(state.current)?'Cadastre o motorista como Condutor e complete CPF, número da CNH, nome da mãe e nome do pai.':'Cadastre somente pessoas diretamente relacionadas ao fato.',button:driverDataRequired(state.current)?'Adicionar motorista/condutor':'Adicionar pessoa',empty:'Nenhuma pessoa cadastrada.',render:renderPersonItem,filter:person=>person.tipo!=='Testemunha'});
...
function renderPersonItemReview(p) { return `<div><strong>${escapeHtml(p.nome)}</strong><div class="entity-meta"><span class="chip">${escapeHtml(p.tipo)}</span><span class="chip">${escapeHtml(p.vinculo)}</span>${isDriver(p)&&p.cpf?`<span class="chip">CPF ${escapeHtml(p.cpf)}</span>`:''}${isDriver(p)&&(p.numeroCNH||p.numeroDocumento)?`<span class="chip">CNH ${escapeHtml(p.numeroCNH||p.numeroDocumento)}</span>`:''}${isDriver(p)&&p.nomeMae?`<span class="chip">Mãe: ${escapeHtml(p.nomeMae)}</span>`:''}${isDriver(p)&&p.nomePai?`<span class="chip">Pai: ${escapeHtml(p.nomePai)}</span>`:''}</div></div>`; }
...
Informe uma justificativa curta:`) || '').trim();
  ...
  app.querySelectorAll('[data-add-entity]').forEach(button => button.addEventListener('click',()=>{
    const type = button.dataset.addEntity;
    const defaults = type==='people' && driverDataRequired(state.current) && !(state.current.people||[]).some(isDriver) ? {tipo:'Condutor'} : {};
    openEntityDialog(type,null,defaults);
  }));
...
function collectStepIssues(step, record = state.current) {
  ...
  if (step === 1) {
    ...
    if (driverDataRequired(record)) {
      const drivers = people.filter(isDriver);
      if (!drivers.length) add('','Cadastre o motorista como tipo “Condutor”.','Motorista/condutor obrigatório','people');
      else if (!drivers.some(driverDataComplete)) add('','Complete os dados do motorista: nome, CPF, número da CNH, nome da mãe e nome do pai.','Dados da CNH incompletos','people');
    }
    ...
    (record.materials||[]).forEach((material,index)=>{
      ...
      if (material.tipoItem === 'Peça' && !String(material.desenho||'').trim()) add('','Informe o número de desenho/part number da peça.','Número de desenho obrigatório','materials');
  ...
  if (step === 3) {
    const h = record.history || {};
    if (!String(h.desfecho||'').trim()) add('history-end','Informe as providências adotadas e como a situação terminou.');
...
function personForm(p) {
  ...
  return `${personReusePanel()}<div class="entity-context-note"><strong>Identificação da pessoa</strong><span>Para motorista/condutor, os dados da CNH devem ser digitados. Não fotografe a CNH.</span></div><div class="form-grid">
    ...
    <div class="field full driver-field"><div class="entity-context-note compact"><strong>Dados digitados da CNH do motorista</strong><span>Obrigatórios para Condutor: nome, CPF, número da CNH, nome da mãe e nome do pai. A foto da CNH não deve ser utilizada.</span></div></div>
    <div class="field driver-field"><label class="required" for="driver-cpf">CPF do motorista</label><input id="driver-cpf" name="cpf" value="${escapeHtml(p.cpf||'')}" inputmode="numeric" placeholder="000.000.000-00" required></div>
    <div class="field driver-field"><label class="required" for="driver-cnh">Número da CNH</label><input id="driver-cnh" name="numeroCNH" value="${escapeHtml(p.numeroCNH||p.numeroDocumento||'')}" inputmode="numeric" placeholder="Número da CNH" required></div>
    <div class="field driver-field"><label for="driver-cnh-category">Categoria da CNH</label><input id="driver-cnh-category" name="categoriaCNH" value="${escapeHtml(p.categoriaCNH||'')}" placeholder="Ex.: B, C, D ou E" autocapitalize="characters"></div>
    <div class="field driver-field"><label for="driver-cnh-validity">Validade da CNH</label><input id="driver-cnh-validity" name="validadeCNH" type="date" value="${escapeHtml(p.validadeCNH||'')}"></div>
    <div class="field driver-field"><label class="required" for="driver-mother">Nome da mãe</label><input id="driver-mother" name="nomeMae" value="${escapeHtml(p.nomeMae||'')}" required></div>
    <div class="field driver-field"><label class="required" for="driver-father">Nome do pai</label><input id="driver-father" name="nomePai" value="${escapeHtml(p.nomePai||'')}" required></div>
...
function materialForm(m) {
  ...
  return `<div class="entity-context-note"><strong>Cadastro adaptado ao tipo de BO</strong><span>${escapeHtml(policy.guidance)}</span></div><div class="form-grid">
    ...
    <div class="field full material-identification-heading"><div class="entity-context-note compact"><strong>Identificação técnica</strong><span>Para peça, o número de desenho/código é obrigatório. Nos demais itens, informe identificações técnicas quando disponíveis.</span></div></div>
    <div class="field material-piece-field"><label class="required" for="material-drawing">Número de desenho / código da peça</label><input id="material-drawing" name="desenho" value="${escapeHtml(m.desenho||'')}" placeholder="Ex.: 735682194" required><small>Obrigatório para peça. Informe o número de desenho, part number ou código equivalente.</small></div>
...
function bindDialogDynamic(type) {
  if(type==='people'){
    ...
    const typeSelect=dialogBody.querySelector('#person-type');
    ...
    dialogBody.querySelector('[data-reuse-person]')?.addEventListener('click',()=>{
      ...
      const mapping = {nome:'person-name',vinculo:'person-link',empresa:'person-company',matricula:'person-reg',setor:'person-sector',telefone:'person-phone',tipoDocumento:'person-doc-type',numeroDocumento:'person-doc-number',dadosComplementares:'person-details',cpf:'driver-cpf',numeroCNH:'driver-cnh',categoriaCNH:'driver-cnh-category',validadeCNH:'driver-cnh-validity',nomeMae:'driver-mother',nomePai:'driver-father'};
    ...
    const update=()=>{
      ...
      const driver=typeSelect?.value==='Condutor';
      ...
      dialogBody.querySelectorAll('.driver-field').forEach(el=>el.classList.toggle('hidden',!driver));
      ['#driver-cpf','#driver-cnh','#driver-mother','#driver-father'].forEach(sel=>{const el=dialogBody.querySelector(sel);if(el)el.required=driver;});
      dialogBody.querySelectorAll('.external-field').forEach(el=>el.classList.toggle('hidden',!external || driver));
    ...
    select.addEventListener('change',update);typeSelect?.addEventListener('change',update);name?.addEventListener('input',update);update();
    ...
    const cpf=dialogBody.querySelector('#driver-cpf');cpf?.addEventListener('input',()=>{let n=cpf.value.replace(/\D/g,'').slice(0,11);cpf.value=n.length>9?`${n.slice(0,3)}.${n.slice(3,6)}.${n.slice(6,9)}-${n.slice(9)}`:n.length>6?`${n.slice(0,3)}.${n.slice(3,6)}.${n.slice(6)}`:n.length>3?`${n.slice(0,3)}.${n.slice(3)}`:n;});
    const cnh=dialogBody.querySelector('#driver-cnh');cnh?.addEventListener('input',()=>cnh.value=cnh.value.replace(/\D/g,'').slice(0,11));
  }
  ...
  if(type==='materials'){
    ...
    const qty=dialogBody.querySelector('#material-quantity'),status=dialogBody.querySelector('#material-quantity-status'),reason=dialogBody.querySelector('#material-quantity-reason'),itemType=dialogBody.querySelector('#material-type'),otherType=dialogBody.querySelector('#material-type-other'),drawing=dialogBody.querySelector('#material-drawing');
    const update=()=>{
      ...
      if (drawing) drawing.required = selected === 'Peça';
...
async function saveDialogEntity(event) {
  ...
  if(type==='people'){
    ...
    formData.numeroCNH=String(formData.numeroCNH||'').replace(/\D/g,'');
    if(formData.tipo==='Condutor'){
      if(formData.cpf.length!==11){await openAppModal({kind:'warning',eyebrow:'Dados do motorista',title:'CPF obrigatório',message:'Informe os 11 dígitos do CPF do motorista.',confirmText:'Completar cadastro'});return;}
      if(!formData.numeroCNH){await openAppModal({kind:'warning',eyebrow:'Dados do motorista',title:'Número da CNH obrigatório',message:'Digite o número da CNH. Não utilize foto do documento.',confirmText:'Completar cadastro'});return;}
      if(!String(formData.nomeMae||'').trim() || !String(formData.nomePai||'').trim()){await openAppModal({kind:'warning',eyebrow:'Filiação do motorista',title:'Filiação obrigatória',message:'Informe o nome da mãe e o nome do pai do motorista.',confirmText:'Completar cadastro'});return;}
      formData.tipoDocumento='Carteira Nacional de Habilitação (CNH)';
      formData.numeroDocumento=formData.numeroCNH;
    }
  ...
  if(type==='materials'){
    ...
    if(formData.tipoItem==='Peça'&&!String(formData.desenho||'').trim()){await openAppModal({kind:'warning',eyebrow:'Identificação da peça',title:'Número de desenho obrigatório',message:'Informe o número de desenho, part number ou código equivalente da peça.',confirmText:'Completar cadastro'});return;}
    ...
    // Materiais/peças ficam ligados diretamente ao BO; não exigem vínculo individual com uma pessoa.
    formData.pessoaId='';
    formData.pessoaNome='';
  }
  if(type==='documents'){
...
function recordMatches(record,rawTerm){
  ...
  const haystack=[record.numero,record.numeroTemporario,record.basic?.referencia,record.basic?.referenciaOutra,record.basic?.origemOcorrencia,record.basic?.diretoria,record.basic?.diretoriaOutra,record.basic?.nomeEmissor,record.basic?.local,record.basic?.complementoLocal,selectedSubmodel(record)?.label,...(record.people||[]).map(p=>`${p.nome} ${p.matricula} ${p.numeroDocumento} ${p.cpf||''} ${p.numeroCNH||''} ${p.nomeMae||''} ${p.nomePai||''}`),...(record.vehicles||[]).map(v=>`${v.placa} ${v.marca} ${v.modelo}`),...(record.materials||[]).map(m=>`${m.tipoItem} ${m.tipoItemOutro} ${m.denominacao} ${m.desenho} ${m.numeroSerie} ${m.codigoIdentificacao} ${m.fornecedor}`),...(record.documents||[]).map(d=>`${d.tipo} ${d.numero}`)].join(' ').toLowerCase();
...
function buildPrintDocument(record) {
  ...
  const hasRequester = requesterRequired(r);
  const round = isRoundOrigin(r);
  const people = (r.people||[]).map(p=>`<article><strong>${printValue(p.nome,'Pessoa não identificada')}</strong><span>${[p.tipo,p.vinculo,p.matricula,p.empresa,isDriver(p)&&p.cpf?`CPF ${p.cpf}`:'',isDriver(p)&&(p.numeroCNH||p.numeroDocumento)?`CNH ${p.numeroCNH||p.numeroDocumento}`:'',isDriver(p)&&p.nomeMae?`Mãe: ${p.nomeMae}`:'',isDriver(p)&&p.nomePai?`Pai: ${p.nomePai}`:''].filter(Boolean).map(escapeHtml).join(' • ')}</span></article>`);
